...
float4 MainPS(VertexShaderOutput input) : SV_Target0
{
	...
	float aa = max(fwidth(dist), 0.0001f);
	float coverage = 1.0f - smoothstep(-aa, 0.0f, dist);

	float4 texColor = SpriteTexture.Sample(SpriteTextureSampler, input.TextureCoordinates);
