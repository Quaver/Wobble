...
namespace Wobble.Graphics.Sprites.Text
{
    ...
    internal unsafe class IndexedStbTrueTypeFontSource : IFontSource
    {
        ...
        public IndexedStbTrueTypeFontSource(byte[] data, IndexedFontSettings settings)
        {
            if (data == null)
            ...
            var offset = stbtt_GetFontOffsetForIndex(dataPointer, settings.Index);

            if (offset < 0)
                throw new ArgumentOutOfRangeException(nameof(settings.Index), settings.Index, "The font does not contain this face index.");
